...
class RobotyController extends baseController {
    ...
    public function add() {
        ...
        if (isset($_SESSION['RobotPelny'])) {          
            ...
            $this->registry->template->narzzedzia = $narzedzia;
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($error)) {
                for ($i = 0; $i < count($narzedzia); $i++) {
                    $p = $narzedzia[$i];
                    $il = $ilosci[$i];
                    $narzedziaZamowienia = new NarzedziaZamowienia();
                    $narzedziaZamowienia->setIlosc($il);
                    $narzedziaZamowienia->setNarzedzia($p);
                    $narzedziaZamowienia->setIdNarzedzia($p->getIdNarzedzia());
                    $narzedziaZ[] = $narzedziaZamowienia;
                }
